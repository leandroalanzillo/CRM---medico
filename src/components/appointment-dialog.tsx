...
export function AppointmentDialog({
...
}: { open: boolean; onOpenChange: (v: boolean) => void; defaultStart?: string }) {
  ...
  useEffect(() => { if (open && defaultStart) setStart(defaultStart); }, [open, defaultStart]);
  useEffect(() => {
    if (notifSettings) {
  ...
  const { data: patients } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("patients").select("id, full_name").eq("clinic_id", clinic!.id).order("full_name");
      return data ?? [];
    },
  });
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    if (!clinic || !patientId || !professionalId || !start) return toast.error("Preencha paciente, profissional e horário.");
    setLoading(true);
    ...
    const { data: conflicts } = await supabase.from("appointments")
      .select("id").eq("professional_id", professionalId)
      .lt("starts_at", endDate.toISOString()).gt("ends_at", startDate.toISOString())
    ...
    const { data: created, error } = await supabase.from("appointments").insert({
      clinic_id: clinic.id, patient_id: patientId, professional_id: professionalId,
      procedure_id: procedureId || null, starts_at: startDate.toISOString(), ends_at: endDate.toISOString(),
    ...
    await addTimeline({ clinicId: clinic.id, patientId, eventType: "appointment",
      description: `Consulta agendada para ${startDate.toLocaleString("pt-BR")}.`, actorId: userId });
    await addAudit({ clinicId: clinic.id, userId, action: "create", resourceType: "appointment", resourceId: created.id });
    // AUTOMATION: move card to "Consulta Agendada"
    await moveCardToStageBySlug({ clinicId: clinic.id, patientId, slug: "agendada",
      reason: "após criação do agendamento", actorId: userId, auto: true });
    ...
    if (notifyEmail || notifyWhatsapp) {
      try {
        const res = await notifyConfirmation({
          data: { appointmentId: created.id, emailPatient: notifyEmail, whatsappPatient: notifyWhatsapp },
        });
        if (res.sent > 0) toast.success(`Confirmação enviada ao paciente (${res.sent}).`);
        else if (res.failed > 0) toast.warning("Agendamento criado, mas falha ao notificar o paciente.");
      } catch {
        ...
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="space-y-3 rounded-lg border bg-muted/30 p-3">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="notify-email" className="font-normal text-muted-foreground">Por e-mail</Label>
              <Switch id="notify-email" checked={notifyEmail} onCheckedChange={setNotifyEmail} />
            </div>
            <div className="flex items-center justify-between">
              <Label htmlFor="notify-wpp" className="font-normal text-muted-foreground">Por WhatsApp</Label>
              <Switch id="notify-wpp" checked={notifyWhatsapp} onCheckedChange={setNotifyWhatsapp} />
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>Cancelar</Button>
            <Button type="submit" disabled={loading}>{loading && <Loader2 className="size-4 animate-spin" />} Agendar</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
