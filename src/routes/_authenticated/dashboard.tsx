...
import { Users, CalendarCheck, CalendarX, TrendingUp, Handshake, Percent, UserPlus, Activity } from "lucide-react";
...
function DashboardPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [patients, appts, negs] = await Promise.all([
        supabase.from("patients").select("id, kind, created_at, professional_id, source").eq("clinic_id", cid),
        supabase.from("appointments").select("id, status, produced_value, professional_id, starts_at").eq("clinic_id", cid),
        supabase.from("negotiations").select("id, status, final_value, created_at").eq("clinic_id", cid),
      ]);
      return { P: patients.data ?? [], A: appts.data ?? [], N: negs.data ?? [] };
    },
  });
  ...
  const m = useMemo(() => {
    ...
    const N = data.N.filter((n) => inRange(n.created_at));
    ...
    const attendance = finished.length + cancelled > 0 ? Math.round((finished.length / (finished.length + cancelled)) * 100) : 0;
    const converted = P.filter((p) => p.kind === "patient").length;
    const conversion = P.length > 0 ? Math.round((converted / P.length) * 100) : 0;
    const inNeg = N.filter((n) => ["negotiating", "awaiting"].includes(n.status));
    const negValue = inNeg.reduce((s, n) => s + Number(n.final_value ?? 0), 0);
    const production = finished.reduce((s, a) => s + Number(a.produced_value ?? 0), 0);

    // Production per professional
    const byProf = (professionals ?? []).map((p, i) => ({
      name: p.name.split(" ")[0],
      value: finished.filter((a) => a.professional_id === p.id).reduce((s, a) => s + Number(a.produced_value ?? 0), 0),
    ...
    return {
      total: P.length, newLeads: newLeads.length, scheduled, finished: finished.length, cancelled,
      attendance, conversion, inNeg: inNeg.length, negValue, production, byProf, byStatus, days,
    };
  }, [data, start, end, prof, professionals]);

  return (
    <div>
      <PageHeader title={`Olá, ${profile?.full_name?.split(" ")[0] ?? ""}`} description="Visão executiva da clínica." />
      ...
      {isLoading || !m ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard label="Consultas agendadas" value={m.scheduled} icon={CalendarCheck} accent="info" />
            <StatCard label="Consultas realizadas" value={m.finished} icon={Activity} accent="success" />
            <StatCard label="Cancelamentos/faltas" value={m.cancelled} icon={CalendarX} accent="destructive" />
            <StatCard label="Comparecimento" value={`${m.attendance}%`} icon={Percent} accent="success" />
            <StatCard label="Conversão" value={`${m.conversion}%`} icon={TrendingUp} accent="primary" />
            <StatCard label="Em negociação" value={m.inNeg} hint={brl(m.negValue)} icon={Handshake} accent="warning" />
          </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="h-72">
                {m.byStatus.length === 0 ? <EmptyChart /> : (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={m.byStatus} dataKey="value" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={2}>
                        {m.byStatus.map((r, i) => <Cell key={i} fill={r.fill} />)}
                      </Pie>
                ...
                <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1">
                  {m.byStatus.map((r) => (
                    <span key={r.name} className="flex items-center gap-1 text-xs text-muted-foreground">
                      <span className="size-2 rounded-full" style={{ background: r.fill }} />{r.name} ({r.value})
                    </span>
                  ))}
                </div>
              </CardContent>
          ...
          <Card className="mt-4">
            <CardHeader><CardTitle className="text-base">Novos leads no período</CardTitle></CardHeader>
            <CardContent className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={m.days} margin={{ left: 8, right: 8 }}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid var(--border)" }} />
                  <Area type="monotone" dataKey="leads" stroke="var(--chart-1)" fill="url(#leadFill)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
