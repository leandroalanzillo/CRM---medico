...
const NEXT: Record<string, string> = { scheduled: "confirmed", confirmed: "in_progress", in_progress: "finished" };
...
function AgendaPage() {
  ...
  function shift(d: number) {
    const nd = new Date(day); nd.setDate(nd.getDate() + d); setDay(nd.toISOString().slice(0, 10));
  }

  async function advance(a: { id: string; status: string; patient: { id: string } | null }) {
    const next = NEXT[a.status];
    if (!next || !clinic) return;
    await supabase.from("appointments").update({ status: next as never }).eq("id", a.id);
    await supabase.from("appointment_status_history").insert({
      clinic_id: clinic.id, appointment_id: a.id, from_status: a.status as never, to_status: next as never, changed_by: userId,
    });
    if (a.patient) {
      if (next === "in_progress")
        await moveCardToStageBySlug({ clinicId: clinic.id, patientId: a.patient.id, slug: "em_consulta", reason: "atendimento iniciado", actorId: userId, auto: true });
      if (next === "finished") {
        await addTimeline({ clinicId: clinic.id, patientId: a.patient.id, eventType: "appointment", description: "Consulta finalizada.", actorId: userId });
        await moveCardToStageBySlug({ clinicId: clinic.id, patientId: a.patient.id, slug: "negociacao", reason: "consulta finalizada com proposta pendente", actorId: userId, auto: true });
      }
    }
  ...
  return (
    <div>
      <PageHeader title="Agenda" description="Agendamentos dos profissionais por dia."
        actions={<Button onClick={() => setOpen(true)}><Plus className="size-4" /> Agendar</Button>} />
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Calendar} title="Nenhum agendamento neste dia"
          action={<Button onClick={() => setOpen(true)}><Plus className="size-4" /> Agendar</Button>} />
      ) : (
        <div className="grid gap-3">
          {appts!.map((a) => (
            <Card key={a.id} className="flex items-center gap-4 p-4 shadow-soft"
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">{a.title} · {a.professional?.name}</p>
              </div>
              ...
              {NEXT[a.status] && (
                <Button size="sm" variant="outline" onClick={() => advance(a)}>
                  {a.status === "scheduled" ? "Confirmar" : a.status === "confirmed" ? "Iniciar" : "Finalizar"}
                </Button>
              )}
            </Card>
          ))}
