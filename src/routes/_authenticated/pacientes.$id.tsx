...
import { fmtDate, fmtDateTime, initials, brl, APPOINTMENT_STATUS, NEGOTIATION_STATUS } from "@/lib/format";
import { addTimeline } from "@/lib/crm";
import {
  ArrowLeft, Pencil, Clock, FileHeart, Lock, Calendar, Handshake, Plus, ShieldCheck, Loader2,
} from "lucide-react";
...
function PatientProfile() {
  ...
  const { data: patient, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("patients").select("*, professional:professionals(name, color)").eq("id", id).single();
  ...
  const { data: timeline } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("patient_timeline").select("*").eq("patient_id", id)
  ...
  const { data: appointments } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("appointments")
        .select("*, professional:professionals(name)").eq("patient_id", id)
  ...
  const { data: negotiations } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("negotiations").select("*").eq("patient_id", id)
        .order("created_at", { ascending: false });
      return data ?? [];
    },
  });
  ...
  return (
    <div>
      <Button variant="ghost" size="sm" asChild className="mb-3">
        <Link to="/pacientes"><ArrowLeft className="size-4" /> Pacientes</Link>
      </Button>

      <Card className="mb-6 shadow-soft">
        <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
          <Avatar className="size-16">
            <AvatarFallback className="bg-primary/15 text-primary text-lg font-bold">{initials(patient.full_name)}</AvatarFallback>
          </Avatar>
          ...
          <Button variant="outline" onClick={() => setEdit(true)}><Pencil className="size-4" /> Editar</Button>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview">
          <Card><CardContent className="grid gap-5 p-6 sm:grid-cols-3">
            <Field label="CPF" value={patient.cpf} />
            <Field label="Nascimento" value={patient.birth_date ? fmtDate(patient.birth_date) : null} />
            ...
            <div className="sm:col-span-3"><Field label="Observações" value={patient.notes} /></div>
          </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="timeline">
          {timeline && timeline.length > 0 ? (
            ...
            <EmptyState icon={Clock} title="Sem eventos ainda" description="As interações do paciente aparecem aqui." />
          )}
        </TabsContent>
        ...
        <TabsContent value="records">
          {canViewClinical ? (
            ...
            <EmptyState icon={Lock} title="Acesso restrito"
              ...
              {appointments.map((a) => (
                <Card key={a.id} className="flex items-center justify-between p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{fmtDateTime(a.starts_at)} · {a.professional?.name}</p>
                  </div>
                  <Badge className={APPOINTMENT_STATUS[a.status].className}>{APPOINTMENT_STATUS[a.status].label}</Badge>
                </Card>
              ))}
            </div>
        ...
        <TabsContent value="negs">
          <div className="mb-3 flex justify-end">
            <Button size="sm" onClick={() => setNegOpen(true)}><Plus className="size-4" /> Nova negociação</Button>
          </div>
              ...
              {negotiations.map((n) => (
                <Card key={n.id} className="flex items-center justify-between p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{brl(n.final_value)} · {fmtDate(n.created_at)}</p>
                  </div>
...
function ClinicalSection({ patientId, clinicId }: { patientId: string; clinicId: string }) {
  ...
  useEffect(() => {
    supabase.from("medical_record_access_logs").insert({
      clinic_id: clinicId, patient_id: patientId, user_id: userId, action: "view_record",
    });
  ...
  const { data: record } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("medical_records").select("*").eq("patient_id", patientId).maybeSingle();
      return data;
    },
  });
  ...
  const { data: evolutions } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("clinical_evolutions").select("*, professional:professionals(name)")
  ...
  async function addEvolution() {
    ...
    const { error } = await supabase.from("clinical_evolutions").insert({
      clinic_id: clinicId, patient_id: patientId, author_id: userId, content: content.trim(),
    });
    setSaving(false);
    if (error) return toast.error("Erro ao registrar evolução.");
    await addTimeline({ clinicId, patientId, eventType: "clinical", description: "Nova evolução clínica registrada.", actorId: userId });
    setContent("");
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 rounded-lg border bg-info/5 p-3 text-xs text-muted-foreground">
        <ShieldCheck className="size-4 text-info" /> Dados clínicos sensíveis — todo acesso é registrado. Evoluções não podem ser excluídas.
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <Card><CardHeader><CardTitle className="text-base">Anamnese & histórico</CardTitle></CardHeader>
        ...
        <Card><CardHeader><CardTitle className="text-base">Nova evolução clínica</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Textarea rows={5} placeholder="Descreva a evolução do atendimento..." value={content} onChange={(e) => setContent(e.target.value)} />
            <Button onClick={addEvolution} disabled={saving} className="w-full">
              {saving ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />} Registrar evolução
            </Button>
          </CardContent>
        </Card>
      </div>

      <Card><CardHeader><CardTitle className="text-base">Histórico de evoluções</CardTitle></CardHeader>
        <CardContent>
              ...
              {evolutions.map((e) => (
                <div key={e.id} className="rounded-lg border p-4">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    {e.professional?.name ? `${e.professional.name} · ` : ""}{fmtDateTime(e.created_at)}
                  </p>
                </div>
              ))}
            </div>
          ) : <p className="py-6 text-center text-sm text-muted-foreground">Nenhuma evolução registrada.</p>}
        </CardContent>
      </Card>
    </div>
  );
