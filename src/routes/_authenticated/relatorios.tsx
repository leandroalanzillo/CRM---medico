...
import { Download, BarChart3, FileText, Printer, Receipt, FileSpreadsheet } from "lucide-react";
...
function rangeFor(period: string, cf: string, ct: string) {
  ...
  if (period === "week") start.setDate(start.getDate() - 6);
    ...
    return { start: cf ? new Date(cf + "T00:00:00") : start, end: ct ? new Date(ct + "T23:59:59") : end };
  }
  return { start, end };
}
...
type ApptRow = {
  id: string; status: string; produced_value: number | null;
  professional_id: string; starts_at: string; patient_id: string;
  procedure_id: string | null; notes: string | null;
  patient: { full_name: string; cpf: string | null; address: string | null; email: string | null; phone: string | null; birth_date: string | null } | null;
...
function RelatoriosPage() {
    ...
    queryFn: async () => {
      ...
      const [patients, appts, negs] = await Promise.all([
        supabase.from("patients")
          .select("id, full_name, cpf, address, email, phone, birth_date, kind, created_at, source, professional_id")
          .eq("clinic_id", cid),
        supabase.from("appointments")
          .select("id, status, produced_value, professional_id, starts_at, patient_id, procedure_id, notes, patient:patients(full_name, cpf, address, email, phone, birth_date), procedure:procedures(name, default_price), professional:professionals(name, registration)")
          .eq("clinic_id", cid)
  ...
  const r = useMemo(() => {
    ...
    const P = data.P.filter((p) => inRange(p.created_at) && (profId === "all" || p.professional_id === profId));
    const N = data.N.filter((n) => inRange(n.created_at) && (profId === "all" || (n as { professional_id?: string }).professional_id === profId));
    ...
    const funnel = [
      ...
      { name: "Em negociação", value: N.length, fill: CHART[3] },
      { name: "Fecharam", value: accepted.length, fill: CHART[4] },
    ];

    const sourceMap: Record<string, number> = {};
    P.forEach((p) => { const s = p.source || "Não informado"; sourceMap[s] = (sourceMap[s] ?? 0) + 1; });
    const sources = Object.entries(sourceMap).map(([name, value], i) => ({ name, value, fill: CHART[i % CHART.length] }));

    const profFilter = profId === "all" ? (professionals ?? []) : (professionals ?? []).filter((p) => p.id === profId);
    const ranking = profFilter.map((p) => {
      const rows = finished.filter((a) => a.professional_id === p.id);
      const value = rows.reduce((s, a) => s + Number(a.produced_value ?? 0), 0);
      return { id: p.id, name: p.name, registration: p.registration ?? "", count: rows.length, value, ticket: rows.length ? value / rows.length : 0 };
    ...
    const monthly = new Map<string, { key: string; label: string; realizado: number; agendado: number; count: number }>();
    A.forEach((a) => {
      const d = new Date(a.starts_at);
      const k = monthKey(d);
      const cur = monthly.get(k) ?? { key: k, label: monthLabel(k), realizado: 0, agendado: 0, count: 0 };
    ...
    const invoiceRows = finished
      ...
      .map((a, idx) => ({
        ...
        "Valor (R$)": Number(a.produced_value ?? 0).toFixed(2).replace(".", ","),
    ...
    const forecast = A.filter((a) => a.status !== "cancelled" && a.status !== "no_show" && a.status !== "finished")
      .reduce((s, a) => s + Number(a.produced_value ?? a.procedure?.default_price ?? 0), 0);

    return {
      funnel, sources, ranking, byProcedure, monthlySeries, invoiceRows,
      totalProduction, acceptedValue, forecast,
      appts: A.length, finished, cancelled, P, N,
    };
  }, [data, start, end, professionals, profId]);

  const periodTag = period === "custom" && from && to
    ? `${from}_a_${to}`
    : `${iso(start)}_a_${iso(end)}`;

  const exportAll = () => {
    if (!r) return;
    download(`producao_por_profissional_${periodTag}.csv`, toCSV(r.ranking.map((x) => ({
      Profissional: x.name, Registro: x.registration, Atendimentos: x.count,
      "Ticket medio": x.ticket.toFixed(2).replace(".", ","),
      Producao: x.value.toFixed(2).replace(".", ","),
    }))));
    download(`producao_por_procedimento_${periodTag}.csv`, toCSV(r.byProcedure.map((x) => ({
      Procedimento: x.name, Quantidade: x.count,
      "Ticket medio": x.ticket.toFixed(2).replace(".", ","),
  ...
  return (
    <div>
      <PageHeader
        title="Relatórios"
        description="Indicadores operacionais, comerciais e informações contábeis do período."
        actions={
          <>
      ...
      {isLoading || !r ? (
        <div className="grid gap-4"><Skeleton className="h-24" /><Skeleton className="h-72" /><Skeleton className="h-72" /></div>
      ) : (
        <>
          {/* KPIs */}
          <div className="mb-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card><CardContent className="p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{r.finished.length} atendimento(s) finalizado(s)</p>
            </CardContent></Card>
            <Card><CardContent className="p-5">
            ...
            <Card><CardContent className="p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {r.appts ? `${((r.cancelled.length / r.appts) * 100).toFixed(1)}% dos agendamentos` : "—"}
              </p>
            </CardContent></Card>
          </div>
          ...
          <Tabs defaultValue="overview">
            <TabsList className="mb-4 flex-wrap">
              ...
              <TabsTrigger value="fiscal">Fiscal / Notas</TabsTrigger>
            ...
            <TabsContent value="overview" className="grid gap-4">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={r.funnel} layout="vertical" margin={{ left: 20, right: 16 }}>
                        ...
                        <Bar dataKey="value" radius={[0, 6, 6, 0]}>
                          {r.funnel.map((f, i) => <Cell key={i} fill={f.fill} />)}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={r.sources} dataKey="value" nameKey="name" outerRadius={85}>
                            {r.sources.map((s, i) => <Cell key={i} fill={s.fill} />)}
                          </Pie>
                          <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid var(--border)" }} />
                        </PieChart>
                      </ResponsiveContainer>
              ...
              <Card>
                <CardHeader className="flex-row items-center justify-between space-y-0">
                  ...
                  <Button size="sm" variant="outline" disabled={!r.ranking.length}
                    onClick={() => download(`producao_profissional_${periodTag}.csv`, toCSV(r.ranking.map((x) => ({
                      Profissional: x.name, Registro: x.registration, Atendimentos: x.count,
                      "Ticket medio": x.ticket.toFixed(2).replace(".", ","),
                      Producao: x.value.toFixed(2).replace(".", ","),
                    }))))}>
                  ...
                  {r.ranking.length === 0 ? (
                    ...
                    <Table>
                      <TableHeader><TableRow>
                        <TableHead>#</TableHead><TableHead>Profissional</TableHead><TableHead>Registro</TableHead>
                        <TableHead>Atendimentos</TableHead>
                        <TableHead>Ticket médio</TableHead><TableHead className="text-right">Produção</TableHead>
                      ...
                      <TableBody>
                        {r.ranking.map((x, i) => (
                          <TableRow key={x.id}>
                            ...
                            <TableCell className="text-muted-foreground">{x.registration || "—"}</TableCell>
                            <TableCell>{x.count}</TableCell>
                            <TableCell>{brl(x.ticket)}</TableCell>
                            <TableCell className="text-right font-semibold text-primary">{brl(x.value)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
            ...
            {/* ================= FISCAL ================= */}
            <TabsContent value="fiscal" className="grid gap-4">
              <Card>
                <CardHeader className="flex-row items-center justify-between space-y-0">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Atendimentos finalizados com dados completos do paciente para emissão de NFS-e / recibos.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" disabled={!r.invoiceRows.length}
                ...
                <CardContent className="p-0">
                  {r.invoiceRows.length === 0 ? (
                    <div className="flex flex-col items-center gap-2 py-12 text-sm text-muted-foreground">
                      <FileText className="size-8 opacity-40" /> Nenhum atendimento finalizado no período.
                    </div>
                  ) : (
              ...
              <Card>
                <CardContent className="p-5 text-sm text-muted-foreground">
                  ...
                  <p className="mt-3 text-xs">
                    Formato compatível com importação em sistemas contábeis e emissores de NFS-e (separador ponto-e-vírgula, codificação UTF-8 com BOM).
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            {/* ================= ACCOUNTING ================= */}
            <TabsContent value="accounting" className="grid gap-4">
              <Card>
                <CardHeader className="flex-row items-center justify-between space-y-0">
                  <CardTitle className="text-base">Evolução mensal</CardTitle>
                  <Button size="sm" variant="outline" disabled={!r.monthlySeries.length}
                    onClick={() => download(`evolucao_mensal_${periodTag}.csv`, toCSV(r.monthlySeries.map((m) => ({
                      Mes: m.label,
                      Atendimentos: m.count,
                      "Receita realizada": m.realizado.toFixed(2).replace(".", ","),
                      "Previsto (agenda)": m.agendado.toFixed(2).replace(".", ","),
                    }))))}>
                    <Download className="size-4" /> CSV
                  </Button>
                </CardHeader>
                <CardContent className="h-72">
                  {r.monthlySeries.length === 0 ? (
                    <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sem dados no período.</div>
                  ) : (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={r.monthlySeries} margin={{ left: 20, right: 16 }}>
                        ...
                        <Legend />
                        <Line type="monotone" dataKey="realizado" name="Realizado" stroke={CHART[0]} strokeWidth={2} dot />
                        <Line type="monotone" dataKey="agendado" name="Previsto" stroke={CHART[3]} strokeWidth={2} strokeDasharray="4 4" dot />
                      </LineChart>
                    </ResponsiveContainer>
                  )}
                </CardContent>
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">Livro caixa por competência</CardTitle></CardHeader>
                <CardContent className="p-0">
                  {r.monthlySeries.length === 0 ? (
                    <div className="py-12 text-center text-sm text-muted-foreground">Sem movimentações.</div>
                  ) : (
                    <Table>
                      <TableHeader><TableRow>
                      ...
                      <TableBody>
                        {r.monthlySeries.map((m) => (
                          <TableRow key={m.key}>
                            <TableCell className="font-medium capitalize">{m.label}</TableCell>
                            <TableCell>{m.count}</TableCell>
                            <TableCell className="text-right font-semibold text-primary">{brl(m.realizado)}</TableCell>
                            <TableCell className="text-right text-muted-foreground">{brl(m.agendado)}</TableCell>
                            <TableCell className="text-right">{brl(m.count ? m.realizado / m.count : 0)}</TableCell>
                          </TableRow>
                        ))}
                        <TableRow className="bg-muted/40 font-semibold">
                          <TableCell>Total</TableCell>
                          <TableCell>{r.monthlySeries.reduce((s, m) => s + m.count, 0)}</TableCell>
                          <TableCell className="text-right text-primary">{brl(r.totalProduction)}</TableCell>
                          <TableCell className="text-right">{brl(r.monthlySeries.reduce((s, m) => s + m.agendado, 0))}</TableCell>
                          <TableCell />
                        </TableRow>
                      </TableBody>
                    </Table>
            ...
            <TabsContent value="procedures">
              <Card>
                <CardHeader className="flex-row items-center justify-between space-y-0">
                  ...
                  <Button size="sm" variant="outline" disabled={!r.byProcedure.length}
                    onClick={() => download(`procedimentos_${periodTag}.csv`, toCSV(r.byProcedure.map((x) => ({
                      Procedimento: x.name, Quantidade: x.count,
                      "Ticket medio": x.ticket.toFixed(2).replace(".", ","),
                      Total: x.value.toFixed(2).replace(".", ","),
                    }))))}>
                ...
                <CardContent className="p-0">
                  {r.byProcedure.length === 0 ? (
                    <div className="py-12 text-center text-sm text-muted-foreground">Sem procedimentos finalizados no período.</div>
                  ) : (
                    <Table>
                      <TableHeader><TableRow>
                        <TableHead>Procedimento</TableHead>
            ...
            <TabsContent value="downloads">
              <div className="grid gap-3 sm:grid-cols-2">
                <DownloadCard
                  ...
                  count={r.P.length}
                  onClick={() => download(`pacientes_${periodTag}.csv`, toCSV(r.P.map((p) => ({
                    Nome: p.full_name,
                    CPF: p.cpf ?? "",
                ...
                <DownloadCard
                  ...
                  count={r.appts}
                  onClick={() => download(`agenda_${periodTag}.csv`, toCSV(data!.A.map((a) => ({
                    Data: fmtDate(a.starts_at),
                    Paciente: a.patient?.full_name ?? "",
                    Profissional: a.professional?.name ?? "",
                    Procedimento: a.procedure?.name ?? "",
                    Status: a.status,
                    Valor: Number(a.produced_value ?? 0).toFixed(2).replace(".", ","),
                  }))))}
                />
                <DownloadCard
                ...
                <DownloadCard
                  ...
                  onClick={() => download(`negociacoes_${periodTag}.csv`, toCSV(
                    r.N.filter((n) => n.status === "accepted").map((n) => ({
                      Data: fmtDate(n.created_at),
                      Valor: Number(n.final_value ?? 0).toFixed(2).replace(".", ","),
                      Status: n.status,
                    })),
                  ))}
                />
...
function DownloadCard({
  title, description, count, onClick,
}: { title: string; description: string; count: number; onClick: () => void }) {
  return (
    <Card>
