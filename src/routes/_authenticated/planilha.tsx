...
function PlanilhaPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["planilha", clinic?.id, from, to, statusFilter],
    enabled: !!clinic?.id,
    queryFn: async () => {
      let q = supabase.from("appointments")
        .select("id, patient_id, starts_at, ends_at, professional_id, procedure_id, title, status, notes, patient:patients(id, full_name, phone, email)")
  ...
  const [importing, setImporting] = useState<{ rows: Row[]; mapping: Record<string, string> } | null>(null);

  // Sync fetched → local editable
  useMemo(() => {
    if (data) {
      setRows(data.map((a): Row => ({
        id: a.id,
  ...
  function addRow() {
    ...
    setRows((r) => [{
      ...
      patient_id: null, patient_name: "", patient_phone: "", patient_email: "",
      starts_at: start.toISOString(), ends_at: end.toISOString(),
      professional_id: professionals?.[0]?.id ?? null,
      procedure_id: null, title: "Consulta", status: "scheduled", notes: "",
  ...
  async function removeRow(idx: number) {
    const row = rows[idx];
    if (row._new) { setRows((r) => r.filter((_, i) => i !== idx)); return; }
  ...
  async function ensurePatient(row: Row): Promise<string | null> {
    ...
    if (!name) return null;
    // Try find by phone in same clinic
    if (row.patient_phone) {
      const { data: found } = await supabase.from("patients")
        .select("id").eq("clinic_id", clinic!.id).eq("phone", row.patient_phone).maybeSingle();
      if (found) return found.id;
    }
    const { data: created, error } = await supabase.from("patients").insert({
      clinic_id: clinic!.id, full_name: name,
      phone: row.patient_phone || null, email: row.patient_email || null,
    }).select("id").single();
    if (error) { toast.error(`Paciente "${name}": ${error.message}`); return null; }
    return created.id;
  }
  ...
  async function saveRow(idx: number) {
    ...
    const payload = {
      ...
      procedure_id: row.procedure_id, starts_at: row.starts_at, ends_at: row.ends_at,
      title: row.title || "Consulta", status: row.status as never, notes: row.notes || null,
    };

    if (row._new) {
      const { data: created, error } = await supabase.from("appointments")
        .insert({ ...payload, created_by: userId }).select("id").single();
      if (error) return toast.error(error.message);
      setRows((r) => r.map((x, i) => i === idx ? { ...x, id: created.id, patient_id: patientId, _dirty: false, _new: false } : x));
    } else {
      const { error } = await supabase.from("appointments").update(payload).eq("id", row.id);
      if (error) return toast.error(error.message);
      setRows((r) => r.map((x, i) => i === idx ? { ...x, patient_id: patientId, _dirty: false } : x));
    }
    toast.success("Salvo.");
    queryClient.invalidateQueries();
  }
  ...
  function exportFile(kind: "xlsx" | "csv") {
    ...
    const flat = rows.map((r) => ({
      Paciente: r.patient_name, Telefone: r.patient_phone, Email: r.patient_email,
      Data: new Date(r.starts_at).toLocaleDateString("pt-BR"),
      "Hora início": new Date(r.starts_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
      "Hora fim": new Date(r.ends_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
    ...
    reader.onload = (ev) => {
      try {
        ...
        const parsed: Row[] = json.map((r) => {
          const get = (keys: string[]) => {
            for (const k of keys) {
              for (const key of Object.keys(r)) {
                if (key.toLowerCase().trim() === k.toLowerCase()) return String(r[key] ?? "").trim();
              }
            }
            return "";
          };
          ...
          const prof = professionals?.find((p) => p.name.toLowerCase() === profName.toLowerCase());
          const proc = procedures?.find((p) => p.name.toLowerCase() === procName.toLowerCase());
          const statusLbl = get(["Status", "situacao", "situação"]);
          const statusKey = Object.entries(APPOINTMENT_STATUS).find(([, v]) => v.label.toLowerCase() === statusLbl.toLowerCase())?.[0] ?? "scheduled";
          return {
  ...
  async function confirmImport() {
    ...
    for (const row of importing.rows) {
      try {
        const patientId = await ensurePatient(row);
        if (!patientId || !row.professional_id) { fail++; continue; }
        const { error } = await supabase.from("appointments").insert({
          clinic_id: clinic!.id, patient_id: patientId, professional_id: row.professional_id,
          procedure_id: row.procedure_id, starts_at: row.starts_at, ends_at: row.ends_at,
          title: row.title, status: row.status as never, notes: row.notes || null, created_by: userId,
        });
        if (error) { fail++; console.error(error); } else ok++;
      } catch (e) { fail++; console.error(e); }
    }
    toast.success(`Importados: ${ok}. Falhas: ${fail}.`);
    setImporting(null);
    queryClient.invalidateQueries();
  }
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button onClick={saveAll} disabled={dirtyCount === 0}>
              <Save className="size-4" /> Salvar {dirtyCount > 0 && `(${dirtyCount})`}
            </Button>
          </div>
        }
      />
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            ...
            {STATUSES.map((s) => <SelectItem key={s} value={s}>{APPOINTMENT_STATUS[s].label}</SelectItem>)}
          </SelectContent>
        </Select>
        <Button variant="outline" size="sm" onClick={addRow}><Plus className="size-4" /> Nova linha</Button>
      </div>
        ...
        <Card className="overflow-x-auto shadow-soft">
          <table className="w-full min-w-[1200px] text-sm">
            <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
              <tr>
                <Th>Paciente</Th><Th>Telefone</Th><Th>Email</Th>
                <Th>Início</Th><Th>Fim</Th>
                <Th>Profissional</Th><Th>Procedimento</Th><Th>Status</Th>
                <Th>Observações</Th><Th className="w-24">Ações</Th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={10} className="p-8 text-center text-muted-foreground">Sem agendamentos no período. Clique em "Nova linha".</td></tr>
              )}
              {rows.map((r, idx) => (
                <tr key={r.id} className={r._dirty ? "bg-warning/5" : "hover:bg-muted/30"}>
                  <Td><Cell value={r.patient_name} onChange={(v) => update(idx, { patient_name: v, patient_id: r._new ? null : r.patient_id })} /></Td>
                  <Td><Cell value={r.patient_phone} onChange={(v) => update(idx, { patient_phone: v })} /></Td>
                  <Td><Cell value={r.patient_email} onChange={(v) => update(idx, { patient_email: v })} /></Td>
                  <Td>
                    <Input type="datetime-local" className="h-8 border-0 bg-transparent px-1"
                      value={toLocalInput(r.starts_at)}
                      onChange={(e) => update(idx, { starts_at: new Date(e.target.value).toISOString() })} />
                  </Td>
                  <Td>
                    <Input type="datetime-local" className="h-8 border-0 bg-transparent px-1"
                      value={toLocalInput(r.ends_at)}
                      onChange={(e) => update(idx, { ends_at: new Date(e.target.value).toISOString() })} />
                  </Td>
                  <Td>
                    <select className="h-8 w-full bg-transparent px-1 text-sm"
                  ...
                  <Td>
                    <select className="h-8 w-full bg-transparent px-1 text-sm"
                      ...
                      onChange={(e) => {
                        ...
                        update(idx, { procedure_id: e.target.value || null, title: proc?.name ?? r.title });
                      }}>
                      <option value="">—</option>
                  ...
                  <Td>
                    <select className="h-8 w-full bg-transparent px-1 text-sm"
                      ...
                      {STATUSES.map((s) => <option key={s} value={s}>{APPOINTMENT_STATUS[s].label}</option>)}
                    </select>
                  </Td>
                  <Td><Cell value={r.notes} onChange={(v) => update(idx, { notes: v })} /></Td>
                  <Td>
                    <div className="flex gap-1">
                      {r._dirty && <Button size="icon" variant="ghost" onClick={() => saveRow(idx)} title="Salvar"><Save className="size-4" /></Button>}
                      <Button size="icon" variant="ghost" onClick={() => removeRow(idx)} title="Excluir"><Trash2 className="size-4 text-destructive" /></Button>
                    </div>
                  </Td>
                </tr>
              ))}
      ...
      <Dialog open={!!importing} onOpenChange={(o) => !o && setImporting(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {importing?.rows.length ?? 0} linhas detectadas. Pacientes novos serão criados automaticamente (identificados por telefone quando disponível).
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-80 overflow-auto rounded border">
            <table className="w-full text-xs">
              <thead className="bg-muted"><tr>
                <th className="p-2 text-left">Paciente</th><th className="p-2 text-left">Telefone</th>
                <th className="p-2 text-left">Data</th><th className="p-2 text-left">Profissional</th>
              </tr></thead>
              <tbody>
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmImport}>Importar {importing?.rows.length} linhas</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function Cell({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return <Input value={value} onChange={(e) => onChange(e.target.value)} className="h-8 border-0 bg-transparent px-1 focus-visible:bg-background" />;
}
...
function parseDateTime(date: string, time: string): Date {
  ...
  const t = /^\d{1,2}:\d{2}/.test(time) ? (time.length === 4 ? `0${time}` : time).slice(0, 5) : "09:00";
